    ...
    function doChange(changes: textChanges.ChangeTracker, sourceFile: SourceFile, decl: DeclarationWithType): void {
        if (isFunctionLikeDeclaration(decl) && (getJSDocReturnType(decl) || decl.parameters.some(p => !!getJSDocType(p)))) {
            ...
            for (const param of decl.parameters) {
                const paramType = getEffectiveTypeAnnotationNode(param, /*checkJSDoc*/ true);
                if (paramType) changes.insertTypeAnnotation(sourceFile, param, transformJSDocType(paramType) as TypeNode);
            }
            const returnType = getEffectiveReturnTypeNode(decl, /*checkJSDoc*/ true);
            if (returnType) changes.insertTypeAnnotation(sourceFile, decl, transformJSDocType(returnType) as TypeNode);
        }
        else {
            ...
            changes.insertTypeAnnotation(sourceFile, decl, transformJSDocType(jsdocType) as TypeNode);
        }
    }
